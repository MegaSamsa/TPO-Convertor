...
public class SquareTrans extends AppCompatActivity {
            ...
            public void afterTextChanged(Editable s) {
                if(metr.hasFocus() && TextEditor.on_edit == false) {
                    try {
                        ...
                        Handler(Double.valueOf(metr.getText().toString()) * Math.pow(10, 6));
            ...
            public void afterTextChanged(Editable s) {
                if (kilometr.hasFocus() && TextEditor.on_edit == false) {
                    try {
                        ...
                        Handler(Double.valueOf(kilometr.getText().toString()) * Math.pow(10, 12));
            ...
            public void afterTextChanged(Editable s) {
                if (mile.hasFocus() && TextEditor.on_edit == false) {
                    try {
                        ...
                        Handler(Double.valueOf(mile.getText().toString()) * Math.pow(1609340, 2));
    ...
    private void Handler(Double mm)
    {
        ...
        if (!metr.hasFocus())
            metr.setText(String.valueOf(df.format(mm / Math.pow(10, 6))));
        if (!kilometr.hasFocus())
            kilometr.setText(String.valueOf(df.format(mm / Math.pow(10, 12))));
        if (!mile.hasFocus())
            mile.setText(String.valueOf(df.format(mm / Math.pow(1609340, 2)))); //Считает и устанавливает значения в остальные поля
    }
